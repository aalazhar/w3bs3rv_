...
This project was completed as part of a 42 core curriculum. it was done with [aalazhar](https://github.com/aalazhar) & [HamzaAmeur](https://github.com/HamzaAmeur11)

The goal of the project is to build a C++98 compatible HTTP web server from scratch.
The web server can handle HTTP GET, HEAD, POST, and DELETE Requests, and can serve static files from a specified root directory or dynamic content using CGI. It is also able to handle multiple client connections concurrently with the help of kqueue().
<br>
...
CGI programs are simply scripts that can be written in any programming language, such as Perl, Python, php, or bash, and are typically used to process data submitted by a user through a web browser, or to generate dynamic content on a web page. [this]((http://www.mnuwer.dbasedeveloper.co.uk/dlearn/web/session01.htm)) will to getting started.

we can fork to execute the CGI.
```
